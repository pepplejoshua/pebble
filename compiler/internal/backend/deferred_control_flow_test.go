...
func TestDeferredBlockControlFlowCompilesAndRuns(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"if", "fn main() i32 { defer { if 1 == 1 { print 7; } else { print 8; } } return 0; }", "7\n"},
		{"while", "fn main() i32 { defer { var i i32 = 0; while i < 3 { print i; i = i + 1; } } return 0; }", "0\n1\n2\n"},
		{"loop", "fn main() i32 { defer { loop 0..3 : i { print i; } } return 0; }", "0\n1\n2\n"},
		{"switch", "fn main() i32 { defer { switch 2 { case 1: print 1; case 2: print 2; else: print 0; } } return 0; }", "2\n"},
		{"for", "fn main() i32 { defer { for var i i32 = 0; i < 3; i = i + 1 { print i; } } return 0; }", "0\n1\n2\n"},
	} {
...
func TestDeferredBareControlFlowCompilesAndRuns(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"if", "fn main() i32 { defer if 1 == 1 { print 7; } else { print 8; } return 0; }", "7\n"},
		{"while", "fn main() i32 { var i i32 = 0; defer while i < 3 { print i; i = i + 1; } return 0; }", "0\n1\n2\n"},
		{"loop", "fn main() i32 { defer loop 0..3 : i { print i; } return 0; }", "0\n1\n2\n"},
		{"switch", "fn main() i32 { defer switch 2 { case 1: print 1; case 2: print 2; else: print 0; } return 0; }", "2\n"},
		{"for", "fn main() i32 { defer for var i i32 = 0; i < 3; i = i + 1 { print i; } return 0; }", "0\n1\n2\n"},
	} {
...
func TestDeferredBareControlFlowBreakContinueCompilesAndRuns(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"while-break", "fn main() i32 { var i i32 = 0; defer while i < 5 { if i == 2 { break; } print i; i = i + 1; } return 0; }", "0\n1\n"},
		{"while-continue", "fn main() i32 { var i i32 = 0; defer while i < 5 { i = i + 1; if i == 2 { continue; } print i; } return 0; }", "1\n3\n4\n5\n"},
		{"loop-break", "fn main() i32 { defer loop 0..5 : i { if i == 2 { break; } print i; } return 0; }", "0\n1\n"},
		{"switch-loop-break", "fn main() i32 { defer switch 1 { case 1: { var i i32 = 0; while i < 3 { if i == 1 { break; } print i; i = i + 1; } } else: print 9; } return 0; }", "0\n"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			out := emitAndRunCapture(t, tc.src, false, 0, false)
...
func TestDeferredBareControlFlowNestedAndScopeCompilesAndRuns(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"if-containing-while", "fn main() i32 { defer if 1 == 1 { var i i32 = 0; while i < 2 { print i; i = i + 1; } } return 0; }", "0\n1\n"},
		{"reads-enclosing-local", "fn main() i32 { var x i32 = 10; defer while x < 12 { print x; x = x + 1; } return 0; }", "10\n11\n"},
		{"lifo-with-plain-print", "fn main() i32 { defer while false { print 1; } defer print 2; return 0; }", "2\n"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			out := emitAndRunCapture(t, tc.src, false, 0, false)
