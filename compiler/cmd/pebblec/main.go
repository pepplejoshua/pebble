...
func run(args []string, stdout, stderr io.Writer) int {
	flags := flag.NewFlagSet("pebblec", flag.ContinueOnError)
	...
	if flags.NArg() != 1 {
		fmt.Fprintln(stderr, "usage: pebblec [-o path] <entry.peb>")
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(stderr, "pebblec: cannot resolve entry %q: %v\n", flags.Arg(0), err)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(stderr, "pebblec: cannot initialize type store: %v\n", err)
		return 1
	}
	...
	if entryID == 0 {
		...
		fmt.Fprintln(stderr, "pebblec: no main function found")
		return 1
	}
	...
	if !result.Successful() || diagnostics.Len() > 0 {
		if err := diagnostic.RenderText(stderr, sources, diagnostics.Items()); err != nil {
			fmt.Fprintf(stderr, "pebblec: rendering diagnostics failed: %v\n", err)
		}
		return 1
	}
	unit := result.IR()
	if unit == nil {
		fmt.Fprintln(stderr, "pebblec: internal error: checker returned no typed IR")
		return 1
	}
	...
	if *outputPath != "" {
		...
		if err != nil {
			fmt.Fprintf(stderr, "pebblec: cannot create output %q: %v\n", *outputPath, err)
			return 1
		}
		defer file.Close()
		out = file
	...
	if err := backend.Emit(unit, unit.Snapshot(), entryID, sources, out); err != nil {
		fmt.Fprintf(stderr, "pebblec: emission failed: %v\n", err)
		return 1
	}
	return 0
}
