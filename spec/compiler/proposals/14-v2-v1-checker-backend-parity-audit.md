...
| Array-typed optional/tuple element | n/a | checker gap, found 2026-08-12 during Phase 3 #4 | **Partial** — `?[3]i32` and `([3]i32, i32)` both fail check-time unification (T0505/T0501); not yet isolated to a root cause |
...
| Slice-typed optional payload `?[]T` | n/a | backend gap, found 2026-08-12 during Phase 3 #5 | **Partial/absent** — rejected at `Emit` ("payload type []int is not supported, want a fixed-width integer, bool, tuple, struct, or enum"); the optional payload machinery has no slice case |
