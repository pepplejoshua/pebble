...
| Interpolation | V1 formats string, bool, integer, float, char, enum, struct, and tuple parts and produces a string value | V2 materializes interpolated strings in general value positions and supports bool, integer, float, str, char, plain-enum, and non-nested-struct parts | **Partial** — tuple parts remain open as F5-09; tagged-union and nested-aggregate recursion unscheduled |
...
| General interpolated-string values | **Partial.** General materialization now works, with bool (`395eb2c`), integer (`7e4bcb4`), float (`8dfcf51`), str (`a785060`), char (`170ff96`), plain-enum (`cd5e3c6`), and non-nested-struct (`7696630`) value parts. | F5-09 (tuple); tagged-union and nested-aggregate recursion unscheduled |
