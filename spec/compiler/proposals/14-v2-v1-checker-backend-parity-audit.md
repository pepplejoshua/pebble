...
| Interpolation | V1 formats string, bool, integer, float, char, enum, struct, and tuple parts and produces a string value | V2 materializes interpolated strings in general value positions and supports bool, integer, float, and str parts | **Partial** — `char`, enum, struct, and tuple parts remain open as F5-06 through F5-09 |
...
| General interpolated-string values | **Partial.** General materialization now works, with bool (`395eb2c`), integer (`7e4bcb4`), float (`8dfcf51`), and str (`a785060`) value parts. | F5-06 through F5-09, one part type per task |
