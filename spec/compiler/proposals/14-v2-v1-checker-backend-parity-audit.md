...
| `str`-element slice | n/a | backend gap, found 2026-08-12 during Phase 3 #5 | **Partial/absent** — rejected at `Emit` ("slice element type str is not supported"); a previous investigation's worklog assumed this was a deliberate checker-level rejection (C0601), but it is actually a backend `Emit` rejection — the checker accepts it |
