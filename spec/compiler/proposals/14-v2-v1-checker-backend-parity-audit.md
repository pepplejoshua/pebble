...
| V2 checker accepts string `+`, but backend cannot materialize its `BinaryValue` result | high from a focused existing backend rejection test; this is a V2 checker/backend defect, not a V1 parity feature | decide whether string concatenation remains in V2, then reject earlier or implement it |
...
| `sizeof` a fixed array passes validation but the backend rejects it | high from `validateSizeof` and `sizeofCTypeName` | one compile-to-C reproduction |
...
| **New, found while fixing the row above:** a bare `sizeof` of ANY plain struct or enum (not just tagged unions) also leaves its typedef uncollected when `sizeof` is its only reference in the program — `sizeof Color`/`sizeof Pair` still emit an undeclared `pebble_enum_N_t`/`pebble_struct_N_t`. Same `SizeofType` collection mechanism as the union case, but a distinct, broader bug — not yet fixed, intentionally scoped out of the union fix. | high, confirmed empirically by the same worker while fixing the union case | one plain-struct and one plain-enum bare-`sizeof` reproduction |
| A helper that returns a plain enum or tagged union is classified as a struct result | high from the nominal `isStruct` branch and missing enum-result branch in `helperSignature` | one plain-enum helper reproduction, then one tagged-union helper reproduction |
