...
| Tuple member `.0`, `.1`, and so on | V1 and V2 resolve tuple ordinals | **Verified** for integer/bool elements (`f1841e1`) across 2-5-element tuples, locals, parameters, and struct fields, and for char/str elements (`bd84ee9`). `f64`/`f32` tuple-ordinal reads now work (`e5add48`, task #86 slice 86a) — the tuple/array TYPEDEF and construction already accepted float elements; only the by-value read was missing a `Load` case in `buildFloatExpr`. Struct field float support remains open (task #86, in progress). |
...
| Tuple call-result/field-read/deref-read as argument | n/a | checker-accepted, was Emit-rejected | **Resolved** (`181f7c6`) — the tuple-side mirror of the row above. `buildAggregateArgument`'s tuple branch gained the same `DirectCall`/`MethodCall` case plus a `Load` case handling both `FieldPlace` and `DereferencePlace`. NEW FINDING (untracked): constructing a struct with an inline tuple-literal field (`Holder.{ p = (20, 22) }`) emits a compound literal typed with the literal's own structural tuple type ID instead of the field's declared type, failing `cc` even with no call involved |
...
| Tuple/struct-field `f64` member | rejected entirely — `f64` is not accepted as a struct field type at all, and by extension a tuple element read-back; `f32`/`f64` were only ever wired up for helper parameters/results (task #22), never aggregate members | **In progress**, task #86, sliced by aggregate kind. Tuple/array element read: **Resolved** (`e5add48`, slice 86a) — typedef/construction already worked, only `buildFloatExpr`'s missing `Load` case was the gap. Struct field: still rejected (`structFieldCType` has no float case) — next slice. Optional payload: still rejected (`optionalPayloadCType` has no float case). Slice element: construction itself is rejected ("slice element type f64 is not supported") — a separate, not-yet-sliced gap |
