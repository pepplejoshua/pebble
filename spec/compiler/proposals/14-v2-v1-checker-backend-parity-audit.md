...
| F6-02 | An explicit `as <T>` cast on a direct function-call result's local initializer is silently dropped | Found 2026-08-14 during the `std/mem/arena.peb` rewrite's own verification (worked around there, not fixed — not independently reproduced or root-caused yet). Reported shape: `let x *MemHeader = some_call(...) as *MemHeader;` where `some_call` returns a related-but-different pointer type (e.g. `*u8`) emits C using the callee's OWN return type at the assignment, not the cast's destination type — an `-Werror -Wincompatible-pointer-types` compile failure. Worked around by giving the callee itself the correctly-typed return type (a function-boundary conversion) instead of relying on the call-site cast. | Not started — needs independent reproduction with a minimal repro (isolated from arena) before scoping an implementation slice. | — |
