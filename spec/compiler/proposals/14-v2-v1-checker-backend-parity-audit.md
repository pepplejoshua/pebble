...
| Tuple/struct-field `f64` member | rejected entirely — `f64` is not accepted as a struct field type at all, and by extension a tuple element read-back; `f32`/`f64` were only ever wired up for helper parameters/results (task #22), never aggregate members | **In progress**, task #86, sliced by aggregate kind. Tuple/array element read: **Resolved** (`e5add48`, slice 86a). Struct field (typedef, construction, read, write): **Resolved** (`8bd34f6`, slice 86b). Optional payload (typedef, construction, force-unwrap, plus new `pebble_rt_checked_unwrap_f32`/`f64` runtime helpers): **Resolved** (`11e9b83`, slice 86c). Slice element: construction itself is rejected ("slice element type f64 is not supported") — the remaining sub-task, not yet sliced |
