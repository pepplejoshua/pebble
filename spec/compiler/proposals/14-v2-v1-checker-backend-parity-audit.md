...
| Fixed array `[N]T` | V2 has `ArrayValue`, `ArrayRepeat`, array locals, indexing, and `.len` | **Partial.** Struct fields (`9dfa4e1`), local/literal reassignment (`aef808e`), enum elements (`94a2a39`), ordinary `sizeof`, direct array-literal/repeat return (`7c625ab`), `sizeof [N]Struct`/tuple/optional element collection (`cf97cd3`), and array-literal struct-field construction at every non-default integer width, plain and generic (`249995d`) landed. Confirmed working (2026-08-12): local literals/repeats, parameters (including inline literals), returns, and arrays of structs/tuples, at every width. Remaining gaps split into their own rows below: nested arrays, `ArrayRepeat` as a struct-field value, and array-typed optional/tuple elements |
...
| Array-of-arrays struct field | n/a | backend gap, found 2026-08-13 during Phase 3 #31, deliberately deferred | **Partial/absent** — rejected by `orderAggregateTypes`' "more than one level of nesting" depth check before the array typedef machinery is even reached; needs its own depth-check surgery, not just the nested-array element-type fix |
| `ArrayRepeat` of an array-typed value | n/a | backend gap, found 2026-08-13 during Phase 3 #31, deliberately deferred | **Partial/absent** — `[[7,8,9]; 2]` is checker-legal but the backend's repeat-value builder doesn't yet handle an array-typed repeated value |
| Binding a whole inner-array read to a local | n/a | backend gap, found 2026-08-13 during Phase 3 #31, deliberately deferred | **Partial/absent** — `let q [3]int = a[0];` (a is `[N][M]T`) is rejected; the local-initializer Load path only accepts a `DereferencePlace`, needs a `CheckedIndexPlace` case |
| Arithmetic on a non-default-width array element read | n/a | checker gap, found 2026-08-13 during Phase 3 #31, general/pre-existing, not array-specific | **Partial/broken** — `b[0] + 1` where `b` is `[3]i32` fails T0505 when the element read is used directly as an arithmetic operand; a direct return, explicit cast, or binding to a local all work. Never previously surfaced because existing tests always used an `as`-cast. Root cause not yet chased |
...
| Array-typed tuple element | n/a | checker gap, found 2026-08-12 during Phase 3 #4 | **Partial** — `([3]i32, i32)` fails check-time unification (T0501); not yet isolated to a root cause. (The array-typed OPTIONAL half of this row, `?[3]i32`, is resolved — see "Optional `?T`" below: the T0505 checker gap was found already fixed by prior work, and the remaining backend gap closed in `08ce755`) |
...
| Nested anonymous struct literal as a field value (`Outer.{ inner = .{ a = 1 } }`) | n/a | checker gap, found 2026-08-13 during Phase 3 #33, general/pre-existing, affects the named outer form too | **Partial/broken** — fails T0510 (inference variable has no unique semantic type) whether the OUTER construction is named or anonymous; the inner anonymous literal's destination type isn't pinned from the field's declared type. Root cause not yet chased |
| Optional field assigned `some <value>` in a `.{ ... }` literal (`.{ opt = some 5 }`) | n/a | checker gap, found 2026-08-13 during Phase 3 #33, general/pre-existing, affects the named form too | **Partial/broken** — fails C0601 (cannot convert value for field) whether the construction is named or anonymous; a pre-existing optional-field classification limitation. Root cause not yet chased |
| Scalar cast inside a generic function body | n/a | checker gap, found 2026-08-12 during Phase 3 #4, unrelated to arrays | **Partial/broken** — `x as T` (or similar) inside a generic function reports "no valid conversion"; pre-existing, general generic-cast gap, not array-specific |
...
| `.len` read directly on a str-typed indexed lvalue expression | n/a | backend gap, found 2026-08-13 during Phase 3 #32, general/pre-existing, not slice-specific | **Partial/broken** — `s[i].len` (s any str-indexed base, slice or otherwise) emits `pebble_rt_checked_index_` with no width suffix (`checkedSuffix(width)` resolves empty for this call site's width argument), an undeclared-function C compile error. Independently reproduced via a throwaway probe. Workaround: bind/pass the str by value to a helper first, then read `.len` there. Root cause not yet chased |
...
| Tagged union | V2 construction, ordinary switch narrowing, generic-self read/write narrowing, and helper results exist | **Verified** (`9a5342e`) for the payload-width restriction (widened from i32/bool/str-only to every scalar shape — see the ledger row below); generic-self narrowing resolved in `7b7eee0`/`7e7163e`; struct-field construction resolved `e3478af`. Aggregate payload shapes (struct/tuple/array/slice/optional) remain unsupported by a deliberate, documented scope limit (typedef ordering), not a bug. One real gap found and NOT fixed: inline nested-union construction typedef ordering — see the new row below |
| Nested tagged-union payload, inline construction | n/a | backend typedef-ordering gap, found 2026-08-12 during Phase 3 #8 | **Partial** — `Outer.value(Inner.b(7))` (constructing the inner union inline, inside the outer's payload argument, rather than binding it to a local first) fails to compile: the outer union's typedef is emitted before the inner's, because the collection walk only reaches Inner through Outer's payload child in that construction order. The WORKING form (`let i = Inner.b(7); let c = Outer.value(i);`, construct-then-reference) is proven by `TestEmitTaggedUnionNestedUnionPayloadCompileAndRun`. Not fixed — narrowly reproducible, not yet root-caused |
...
| Escape analysis for a stack-local's address | n/a | backend gap, found 2026-08-12 during Phase 3 #3 (pointer/aggregate deref) investigation | **Decision needed** — `fn mk() *T { var x T = ...; return &x; }` is accepted by the checker for any aggregate type (confirmed for both `[N]i32` and a plain struct, not array-specific) and compiles to a real dangling pointer; the test harness's `-Werror` catches it as `-Wreturn-stack-address`, but nothing in the checker rejects the *source* pattern — V1 had no escape analysis either. Not fixed; matches the "Decision needed" class of the Untagged union row above, not a narrowly scoped bug |
...
| Enum call argument from a call result, struct field, or integer-to-enum cast | n/a | backend gap, found 2026-08-12 during Phase 3 #13 | **Partial** — the SAME rejection as the literal-argument row above ("want a reference to an enum-typed local... binding the value into a local first is required") also hits `check(pick())` (a DirectCall), `check(s.c)` (a Load/FieldPlace), and `check(1 as Color)` (a CheckedIntegerToEnum) used directly as a plain-enum call argument. The literal-argument shape was deliberately the only one fixed in Phase 3 #13; these three remain, workaround unchanged (bind to a local first) |
