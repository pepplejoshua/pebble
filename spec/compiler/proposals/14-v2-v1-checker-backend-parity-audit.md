...
| A tagged union used as a struct field or optional payload receives the plain-enum C type name | high from `isEnumType`, `structFieldCType`, and `optionalPayloadCType` ordering | one struct-field compile-run reproduction, then one optional-payload reproduction |
...
| `sizeof` a tagged union selects the discriminant enum C type instead of the tagged-union C type | high from `sizeofCTypeName` branch order | one runtime size comparison reproduction |
