...
| **New, found while verifying the row above:** a negative integer literal in a switch case label (`case -5:`) is rejected outright — `switch case contains an integer literal with malformed text "-5"` — for a signed subject type (e.g. `i16`). `buildCaseLabel`'s `isNonNegativeDecimal` guard has no negative-literal path at all. Pre-existing, unrelated to the width-widening fix above (surfaced only while verifying it against a signed subject). | high, directly reproduced | one checker/backend reproduction: a signed-integer switch subject with a negative case label, isolate whether the checker even builds a negative `CaseValue` literal correctly before touching the backend |
| **New, found while verifying the switch-subject-widening row above:** the abstract `uint` builtin is still rejected as a switch subject at the backend (`switch subject has type uint, want int, bool, or char, or an enum/tagged-union type`) — pre-existing, not a regression from the fixed-width-integer widening. | medium, directly reproduced but not root-caused (uint may need different width-resolution handling than the other fixed-width builtins, since it has no fixed bit-width of its own) | one reproduction isolating why uint doesn't fit the same resolvedBuiltin/cType path the other fixed-width integers now use |
...
| — struct slice: **RESOLVED (`9df0351` local/literal, `5ef060a` call value).** Whole struct-local reassignment now works for a pointer-deref/field write and a plain local, with the new value an in-scope struct-typed local, a fresh struct literal, OR a call to a struct-returning helper (`p = make_point();` / `*self = make_point();`) — all reproductions independently verified end-to-end (exit 9, causation-checked). Tuple, array, enum, and string reassignment remain untouched. | — |
