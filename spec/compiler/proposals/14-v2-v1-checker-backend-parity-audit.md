...
| Interpolation | V1 formats string, bool, integer, float, char, enum, struct, and tuple parts and produces a string value | V2 materializes interpolated strings in general value positions and supports bool, integer, float, str, char, and plain-enum parts | **Partial** — tagged-union, struct, and tuple parts remain open as F5-08/F5-09 (plus tagged-union recursion, unscheduled) |
...
| General interpolated-string values | **Partial.** General materialization now works, with bool (`395eb2c`), integer (`7e4bcb4`), float (`8dfcf51`), str (`a785060`), char (`170ff96`), and plain-enum (`cd5e3c6`) value parts. | F5-08/F5-09, one part type per task; tagged-union recursion unscheduled |
