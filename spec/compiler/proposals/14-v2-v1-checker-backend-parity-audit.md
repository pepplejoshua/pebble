...
| Interpolation | V1 formats string, bool, integer, float, char, enum, struct, and tuple parts and produces a string value | V2 materializes interpolated strings in general value positions and supports bool, integer, float, str, and char parts | **Partial** — enum, struct, and tuple parts remain open as F5-07 through F5-09 |
...
| General interpolated-string values | **Partial.** General materialization now works, with bool (`395eb2c`), integer (`7e4bcb4`), float (`8dfcf51`), str (`a785060`), and char (`170ff96`) value parts. | F5-07 through F5-09, one part type per task |
