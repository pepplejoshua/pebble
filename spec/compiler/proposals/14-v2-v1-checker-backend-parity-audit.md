...
| Untagged union | V1 emits a C union and permits construction and member access | V2 rejects construction, read, and write because no safety rule is accepted | **Decided 2026-08-13** — support as explicitly unsafe: no runtime tag, no discriminant tracking, direct C `union` semantics matching V1 exactly (reading a field other than the one last written reinterprets the same bytes, same as real C — the checker performs no cross-field safety check at all). Users wanting safety should use a tagged union instead. Root-caused: construction is rejected by `aggregate_validation.go`'s `aggregateStruct` case hard-requiring `declaration.Nominal == infer.NominalStruct`; member read/write is rejected by `member_validation.go`'s field-lookup loop, which never matches a `NominalUnion` declaration's members. **In progress** (Phase 3 #51) |
