...
| Non-default-width value returned from a default-width-returning function | n/a | backend gap, found 2026-08-13 during Phase 3 #41, general/pre-existing | **Partial/broken** — `fn main() int { var x u8 = 200; return x; }` (no arithmetic at all) fails Emit with "...contains a SymbolValue of type u8, want int"; confirmed pre-existing and unrelated to the Phase 3 #41 checker fix (which correctly makes the checker ACCEPT this and similar arithmetic-result shapes, exposing this separate backend limitation as newly reachable). Root cause not yet chased |
| `TestEmitRejectsSliceParameterUnsupportedElementType` hand-built-IR test failure | n/a | test/backend gap, found 2026-08-13 during Phase 3 #41's full-suite checkpoint, pre-existing | **Partial/broken** — the hand-built IR fixture (`buildSliceOfStrParameterUnit`) produces a call node with 0 arguments instead of 1, so the test's assertion never reaches the "unsupported element type" rejection it's meant to exercise; confirmed via causation-check to fail identically on unmodified `HEAD`, unrelated to any Phase 3 #41 change. Root cause not yet chased |
