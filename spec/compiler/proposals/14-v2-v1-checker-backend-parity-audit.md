...
|---|---|---|
| **New, found while verifying composite print slice 3 (proposal 17):** a fixed-array-typed struct field is rejected entirely by the backend (`struct type pebble_struct_..._t: field type [3]int is not supported, want a fixed-width integer, bool, str, tuple, struct, enum, pointer, slice, function type, or runtime type`) — confirmed even without `print` involved (a bare construct-and-index reproduction fails identically), so this is a pre-existing gap in struct-field-type support, unrelated to the print feature that surfaced it. | high, directly reproduced | one reproduction: a struct with a fixed-array field, constructed and indexed with no print statement; find where `structFieldCType` (or equivalent) enumerates accepted field types and add the missing array case, mirroring how slice-typed struct fields were presumably added |
