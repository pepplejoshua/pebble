...
| Enum variant literal as a direct call argument | n/a | backend gap, found 2026-08-12 during Phase 3 #1 (module-level constant) investigation | **Partial** — `check(Color.green)` is rejected: "call to symbol N parameter 0 ... is a EnumVariantValue, want a reference to an enum-typed local ... (binding the value into a local first is required)". Confirmed general, not constant-specific, via a plain non-constant control case. Workaround: bind the variant to a local first (`var c Color = Color.green; check(c);`). Not yet fixed |
