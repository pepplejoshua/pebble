...
| Bare string-literal arrow body | n/a | checker bug | **New gap** (found during batch J, `264ad45`): a named function or method whose `=>` expression body is a bare string literal is wrongly rejected with T0501 "unknown calling convention". `infer/declaration.go`'s `prepareSignatures()` -> `convention()` (`declaration.go:338-360`) scans every `StringLiteral` child of the `FunctionDecl` node to detect a leading C/Pebble convention annotation, and a string-literal expression body is itself a direct `StringLiteral` child, so it's misidentified as a malformed convention annotation. Repro: `fn f() str => "hello";` and `fn tag(self Box) str => "b";`. Non-literal `=>` str bodies and block-body str returns are unaffected |
...
| Struct call-result/field-read as argument or receiver | n/a | checker-accepted, Emit-rejected | **New gap** (found during batch I, `ed48868`): `buildStructArgument` (`calls.go:1601-1606`) accepts a whole-struct argument or method receiver only as an in-scope local, an inline `RecordConstruct`, `context`, or `Load(DereferencePlace)`. A struct-typed CALL RESULT used directly (`read(mk())`, `mk().get()`) or a struct-typed FIELD READ used directly (`read(h.p)`, `h.p.get()`) fails at Emit with "want a reference to a struct-typed local..." / "want a DereferencePlace". Only slice/str field-as-argument shapes are otherwise tracked in this audit |
...
| Negative-MIN literal for `i32`/`i64`/`int` (`-2147483648`, `-9223372036854775808`) | checker-accepted (correctly — these are the valid signed minimums), but the checked-negation lowering emits the POSITIVE magnitude (`2147483648`, `9223372036854775808`) as the runtime helper's constant argument, and that magnitude is itself unspellable as a signed C literal at the destination width, so `cc` fails under `-Werror` (`-Wconstant-conversion`/`-Wimplicitly-unsigned-literal`); `i8`/`i16` minimums work fine via a separate constant-fold path | **Confirmed absent**, discovered during literal proof-batch verification (2026-08-11), queued as a routine gap-fix task (narrow, well-scoped — the checked-negation MIN-literal lowering needs to spell the destination-width minimum directly rather than negating an unspellable positive magnitude) |
| Void-result anonymous function whose body can fall through (`fn () void {}`) | V1 accepts this (`checker.c:1392` gates the fall-through error on `return_type->kind != TYPE_VOID`); V2 wrongly rejects it with C0607 ("non-void function can fall through without returning"), even though a NAMED void helper (`fn helper() void {}`) is accepted fine. Root cause: `infer/declaration.go`'s `prepareSignatures()` skips anonymous-function (`syntax.FunctionTerm`) symbols entirely, so no signature is ever recorded for one; `check/control_flow_validation.go`'s `isVoidResult()` then fails that signature lookup and treats the anonymous function as non-void. An explicit trailing `return;` works around it — only the void-DETECTION path is broken, not void handling generally. | **Confirmed absent**, discovered during function proof-batch verification (2026-08-11), queued as a routine gap-fix task (narrow — `prepareSignatures()` needs to record a signature for anonymous-function symbols too, or `isVoidResult()` needs a fallback path that doesn't depend on that lookup) |
