...
| **New, found while fixing the row above:** an enum/tagged-union-returning `DirectCall` used directly in a general value position — not a tail-position `return`, not a local-declaration initializer — still cleanly rejects from `buildEnumValue`/`buildUnionValueExpr`'s existing default cases. `values.go` was intentionally untouched (out of that dispatch's declared scope). | high, confirmed empirically by the same worker while fixing the row above | one reproduction: an enum/union-returning call used inline as, e.g., a switch subject or comparison operand, not bound to a local first |
