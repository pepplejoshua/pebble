...
| Untagged union | V1 emits a C union and permits construction and member access | V2 rejects construction, read, and write because no safety rule is accepted | **Resolved for scalar payloads** (`46df6e1`, Phase 3 #51) — supported as explicitly unsafe: no runtime tag, no discriminant tracking, direct C `union` semantics matching V1 exactly, confirmed via a genuine bit-pattern reinterpretation test (i32 `-1` read back through a `u32` field as `4294967295`). `internal/tir.TypeDecl` gained a `Union` bool signal since neither the type snapshot nor member-declaration node kinds can otherwise distinguish an untagged union from a tagged union (both parse members as `SymbolVariant`/`VariantDecl`) or a plain struct. Every Nominal-dispatch site the backend already had a struct/enum/tagged-union split for (array element, slice element, pointer pointee, sizeof, struct field, optional payload) gated with a new `isUntaggedUnionType` check before the struct fallthrough. Scoped to scalar fields only (fixed-width integers, uint, bool, char) this slice; struct/array/tuple/optional/str fields inside a union remain unsupported. A narrow, separate gap surfaced and deliberately left unfixed: reading (not writing) a `char`-typed union field fails at Emit — see the new row below |
| `char`-typed untagged-union field read | n/a | backend gap, found 2026-08-13 during Phase 3 #51's test-coverage pass, deliberately deferred | **Partial/broken** — a `char` union field's construction and write both work, but reading it back fails at Emit (`buildStructFieldRead`/`buildStructFieldValueRead` have no `char` case for a union-typed receiver, though the union typedef declares the field as `int32_t` and construction already builds it via `buildCharOperand`). The scalar round-trip coverage in `untagged_union_test.go` stops at `bool` and documents this gap in a comment. Root cause not yet chased |
...
| C variadic extern call | V1 permits primitive C variadic use | V2 reports C0604 | **Decided 2026-08-13**: support a real C-ABI bare `...` (untyped, no name) as the final parameter of an `extern fn` declaration only, with NO interop type checking on the variadic call-site tail — pure unsafe passthrough to the real C variadic ABI, the same "unsafe, no checking, Pebble-land gets its own real checked variadic (`...name []T`, already fully working, including on `extern` declarations) for anyone wanting safety" treatment as the untagged-union decision. Root-caused: the parser has no syntax for a bare, nameless `...` at all (confirmed live — `extern fn printf(fmt *u8, ...) i32;` fails at the PARSER with P0002/P0004, never reaching C0604); Pebble's own typed `...args []int` variadic syntax already compiles cleanly on an extern declaration but does not match C's actual variadic calling convention. **In progress** (Phase 3 #52) |
