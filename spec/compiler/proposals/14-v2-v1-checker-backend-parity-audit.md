...
| Interpolation | V1 formats string, bool, integer, float, char, enum, struct, and tuple parts and produces a string value | V2 supports only bool value parts; as of Phase 3 #36 this is no longer print-only, but general materialization (local/argument/return/etc.) still requires every value part be `bool` | **Partial** — non-bool value-part types remain unsupported (separately tracked) |
...
| Interpolated string with a non-`bool` value part (int, float, str, enum, etc.) | n/a | backend/runtime gap, found 2026-08-13 during Phase 3 #36, deliberately deferred | **Absent** — only literal text and `bool`-typed value parts are supported (both in `buildPrint` and, as of Phase 3 #36, in general materialization); every other interpolated value type is unimplemented, matching a real, much larger formatting-matrix scope deliberately not attempted in one dispatch |
