...
| Nested fixed array `[N][M]T` | n/a | backend gap, found 2026-08-12 during Phase 3 #4 | **Partial/absent** — an array of arrays is rejected by the backend in every position tried (local, parameter, struct field). Not isolated to a specific error yet; needs its own investigation |
| `ArrayRepeat` as a struct-field construction value | n/a | backend gap, found 2026-08-12 during Phase 3 #4 | **Partial** — `Box.{ data = [7; 3] }` fails C0601 for a narrow-width element and fails at `Emit` ("array field initialized from a ArrayRepeat") even for the default `[3]int` width; the array-LITERAL field-construction fix (`249995d`) does not cover the repeat-initializer shape |
