...
| F5-01b | Duplicate C enumerators across two instantiations of one generic tagged union | Found during F5-01's investigation, 2026-08-14. A program with TWO DIFFERENT concrete instantiations of the same generic tagged union live at once (e.g. `Result[int, str]` and `Result[bool, str]` both constructed in one program) emits duplicate C enumerator names for the shared variant names (`pebble_variant_29` twice) — independent of the F5-01 fix, reproduces identically before and after it. Likely the per-instantiation enum-typedef collection doesn't dedupe or uniquely-suffix variant names across separate specializations of the same generic union declaration. | **Resolved** (`4df19bc`) — root cause confirmed: `enumVariantName` named a variant's C enum constant using ONLY the variant's own symbol (`pebble_variant_<memberID>`), correct for a non-generic enum/union but colliding across specializations of a generic one, since they all share the same template's variant symbols. Widened to `pebble_variant_<ownerTypeID>_<memberID>`, mirroring the existing `pebble_enum_<typeID>_t`/`pebble_union_<typeID>_t` convention, threaded through all 13 call sites across 8 files. Verified end-to-end via real `cc` compile-and-run for two live specializations plus a structural test proving distinct constant names; full suite clean at `-parallel 12`, causation-checked against `HEAD`. | — |
...
| F5-06b | `print` truncates any `str` value at an embedded NUL byte | `fprintf(..., "%s", ...)` is a C-string operation and stops at the first `0x00`; confirmed independent of interpolation (`var s str = "x\0y"; print s;` already truncates on plain `HEAD`, no char/interpolation involved). | **Resolved** (`3d5c93a`) — str print operands (bare, and nested in a struct/tuple/array/slice field) now force the sequential-print path (mirroring composite/enum/optional/pointer/function operands via `unwrapPrintOperands`'s `hasComposite` flag) and emit a length-bounded `fwrite((const void *)(expr.data), 1, expr.len, stdout)` raw statement instead of a printf `%s` specifier — new `buildStrPrintOperand`/`buildStrPrintValueCalls`, mirroring the existing pointer-print machinery's shape. The old `buildScalarPrintParts` str case now hard-errors rather than silently reinstating `%s`. Verified byte-exact via `od -c` for both a bare str and a str-typed struct field, full `internal/backend`/`internal/check` suites clean at `-parallel 12`, causation-checked against `HEAD`. This closes the F5 backlog's last remaining item. | — |
...
| F5-22 | Print an optional value | V1 has an optional format path. V2 rejects the operand in the checker. | **Resolved** (`1987102`) — `printableType` gains a `types.Optional` case recursing into the payload (mirroring slice/tuple element recursion); backend emits a raw C `if (<expr>.has_value) { "some(" + <recursive payload> + ")" } else { "none" }`. Widened `buildOptionalValueExpr` to accept `SymbolValue`/`DirectCall`/`SourceAlias` print-operand sources and taught `buildStructValueExpr`/`buildTupleValueExpr` to unwrap a `SourceAlias`-wrapped nested payload literal. Verified with byte-exact captured stdout across scalar/struct/tuple payloads; a pointer-payload optional stays rejected (slice 8, not yet printable). Full `internal/backend` and `internal/check` suites clean. | — |
| F5-23 | Print a pointer value | V1 has a pointer format path. V2 rejects the operand in the checker. | **Resolved** (`a8c48b8`) — `printableType` gains a `types.Pointer` LEAF case (unconditionally printable, no pointee recursion — this is what makes a self-referential cycle trivially safe). Backend emits `"&" + %p` for non-nil, `"nil"` for null. A self-referential struct (`Node { next *Node }`) needed its C typedef to carry a tag name for the pointer field to self-reference before the typedef completes — detected via precise pointer-field cycle detection (`structIsCyclic`), not universal tagging (an earlier broad attempt regressed 22 tests, caught by the full-suite checkpoint and corrected). Two now-obsolete negative print tests (optional/union with a pointer payload) converted to positive tests; a dedicated cycle-termination test added. | — |
| F5-24 | Print a function value | V1 has a function-value format path. V2 rejects the operand in the checker. | **Resolved** (`460a769`) — `printableType` gains a `types.Function` unconditional LEAF case. Backend dispatches on the print operand's TIR node shape: a bare named-function reference prints its declared source name (`<fn f>`); every other shape (local/parameter/field/call-result) prints its raw C function pointer address (`<fn @0x...>`), reusing F5-23's pointer format. A real `-Wunused-function` compile bug (the named case emitted only a string literal, never referencing the underlying C function) was caught and fixed with a `(void)`-cast reference, mirroring this backend's existing unused-variable-suppression convention. This closes proposal 17's composite-print slices in full. | — |
| F5-25 | Platform-sized `int` and `uint` are not implemented end to end | The ABI spec already requires target-native word types. The checker has `LiteralTarget.WordBits`, but `pebc` hardcodes it to 64. The backend then maps `int` to `int32_t` and `uint` to `uint64_t`; compatibility checks and checked-runtime-helper selection repeat those fixed mappings. On the current 64-bit host, `uint` matches by accident and `int` is wrong. `let x int = 2147483648;` passes checking and fails in `cc`. | **Resolved** (`3bae4e7`) — `cType` now maps `types.Int` to `int64_t` (matching the only target this compiler builds for, per `WordBits: 64` hardcoded in `cmd/pebc`/`cmd/tirdump`, matching spec `05-types-and-inference.md`'s target-native-word definition); threaded through `printfSpecifier`, `integerKindRange`, and all five checked-arithmetic/shift/negation/unwrap/float-cast runtime-helper-suffix functions that previously routed `int` through the `i32` family only because they happened to share a C representation. Matching checker-side fix keeps pointer-width and structural-width-comparison logic consistent. The width is still expressed as a hardcoded constant rather than genuinely parameterized by `WordBits` — deliberately deferred, since nothing else in the compiler treats `WordBits` as configurable either and no 32-bit target path exists anywhere; revisit if a real 32-bit target is ever added. Also fixed a related, previously-latent gap found during verification: 64-bit signed literals need an explicit `LL` C suffix (mirroring the existing unsigned `u` suffix), applying to both `int` and `i64`. Took 4 dispatch rounds given the volume of collateral test fallout (91 pre-existing tests initially failed — a mix of stale `int32_t`/width-compatibility assertions needing updates and one genuine backend validation gap that silently relied on int==i32); full `internal/backend` and `internal/check` suites clean on the final round, causation-checked against `HEAD`. | — |
| F6-01 | Brace-less if/loop arm (`else if` and other bare single statements) crashes `Emit` | The grammar documents an if-statement's arms as "a statement" (`spec/compiler/03b-surface-tree.md`, `IfStmt` row), not "a block" — `else if b > 0 { ... }` is unsugared valid syntax whose else arm is a bare `tir.If`, not a `tir.Block`. `buildIf`'s terminal-arm path and `buildFallthroughBody`'s ordinary/loop-body path both required `tir.Block` unconditionally, crashing with `"... is a If, want a Block"`. Found 2026-08-14 while verifying an unrelated `std/mem/arena.peb` rewrite whose `realloc` used `else if`; reproduces independently with no arena involved. | **Resolved** (`bc7f506`) — new `buildIfArm` (terminal if, accepts Block/Return/If) and a non-Block fallback in `buildFallthroughBody` delegating to the existing per-statement dispatcher (covers the general bare-single-statement case: ordinary fallthrough if, loop bodies, loop-if arms). Verified via real compile-and-run of an else-if chain, an else-if-else chain, a bare-return arm, and a bare-if loop body; targeted `TestEmitIf`/`TestEmitWhile` suites clean; causation-checked against `HEAD`. | — |
| F6-02 | An explicit `as <T>` cast on a direct function-call result's local initializer is silently dropped | Found 2026-08-14 during the `std/mem/arena.peb` rewrite's own verification (worked around there, not fixed at the time). Reported shape: `let x *MemHeader = some_call(...) as *MemHeader;` where `some_call` returns a related-but-different pointer type (e.g. `*u8`) emits C using the callee's OWN return type at the assignment, not the cast's destination type — an `-Werror -Wincompatible-pointer-types` compile failure. | **Resolved** (`16ba4e0`) — root cause was `buildPointerLocalDeclaration`'s `tir.PointerCast` case (`internal/backend/locals.go`), which built the cast child's C text and assigned it directly with no cast wrapper, on the mistaken assumption ("C pointer types are already named") that one was unnecessary. Fixed by wrapping the child in an explicit `(<destination>)(...)` cast, matching the shape `buildExpr`'s own `PointerCast` case (ordinary expression positions) already produced correctly — confirmed via grep that no other backend site shares this bug. Verified with a minimal isolated repro (independent of arena) proving the resulting pointer is genuinely usable, not just compile-clean (dereferences to the correct value through the cast), plus a regression test confirming the already-working same-type-family round-trip case (`*i32 -> *void -> *i32`) is unaffected. Causation-checked against `HEAD`. A separate, unrelated minor gap was noticed in passing (not fixed, not yet formal): a same-type pointer "cast" (`p as *u8` where `p` is already `*u8`) is represented as a bare `SourceAlias`, which `buildPointerLocalDeclaration`'s switch has no case for, and cleanly rejects — narrower than this row, may already be intentional (the cast is a semantic no-op), needs its own investigation before it becomes a task. | — |
| F6-03 | Checked `+`/`-`/`*` have no runtime helper at i8/i16/u8/u16/u32 | The checker accepts checked arithmetic at every integer width, but `checkedSuffix` (`internal/backend/operators.go`) only resolved a helper for int/i32/i64/u64 — a narrow-width `+`/`-`/`*` cleanly rejected at Emit rather than compiling (`let c u8 = a + b;` with `a, b u8` failed with "want an operator with a checked runtime helper"). | **Resolved** (`afb8c77`) — added the 30 missing `pebble_rt_checked_add/sub/mul_<width>` runtime functions (SAFE-mode `__builtin_*_overflow` panic, RELEASE-mode wraparound), mirroring the existing i32/i64/u64 pattern exactly; widened `checkedSuffix` to select them. `checkedArithmeticHelper` now explicitly gates `/`/`%` to int/i32/i64 only, so that pair's clean rejection at narrow widths and `u64` is unaffected by the `checkedSuffix` widening — division/modulo at these widths remains a separate, still-open gap (part of the Integer runtime coverage matrix below). Verified via the exact repro (`200u8 + 50u8` → `250`), both runtime build modes compile clean under `-Wall -Wextra -Werror`, targeted backend tests clean, causation-checked against `HEAD`. | — |

The example sweep passes for every example, including `arena_alloc.peb`
(as of `448e386` — rebuilt without pointer arithmetic, its uninitialized
binding fixed in the same change). `read_file.peb` reports its expected
missing demo file and exits successfully.
...
| Enum | V2 construction, switch labels, and integer conversions exist | **Verified** for enum-to-integer and integer-to-enum. Enum-element arrays and slices resolved `94a2a39`; local copy initialization resolved (task #43); ordinary `some Color.red` optional initialization resolved `1bf785d`; enum-typed struct field construction resolved `d19717c` (see line 368 below). Remaining, not yet a formal item: an enum-typed tuple element (construction/read) has not been separately proven — `.0`/`.1` proofs to date cover int/bool/char/str/f32/f64, not enum. |
...
| Function type and function value | V2 supports Pebble-convention, non-variadic function values for a limited C-representable signature set | **Partial**; float (f32/f64) parameter and result support added (`cd6e604`). Plain-struct parameter (`4839a31`, F5-19) and result (`a059058`, F5-20), and `str` result (`1f7939e`, F5-21), are now supported; a struct carrying a function-typed field, or any other non-plain-struct aggregate, remains unsupported. Non-Pebble convention and variadic signatures are intentional/decision-pending scope limits tracked by separate rows (lines 108, 313), not gaps in this row |
...
| Optional, pointer, function value | V1 has format paths for its supported value forms | V2 checker still rejects them | **Resolved; proposal 17 slices 7-9** — optional (`1987102`, F5-22), pointer (`a8c48b8`, F5-23), function value (`460a769`, F5-24). Closes proposal 17's composite-print slices in full |
...
| Reassign a `str` local from another string value | `buildStrStoreValue` accepts only a string literal | **Resolved** (`7d418bd`, F5-03/F5-04) — `buildStoreCore`'s narrow two-case switch was deleted and delegated to `buildStrOperand` (a strict superset already handling `SymbolValue`/`DirectCall`/`MethodCall`) |
...
| Interpolated string with a non-`bool` value part (int, float, str, char, enum, struct, tuple) | n/a | backend/runtime gap, found 2026-08-13 during Phase 3 #36, deliberately deferred | **Resolved** — integer (`7e4bcb4`, Phase 3 #49), float (`8dfcf51`, Phase 3 #50), `str` (`a785060`, F5-05), `char` (`170ff96`, F5-06), plain enum (`cd5e3c6`, F5-07 — a payload-carrying tagged union stays out of scope), non-nested struct (`7696630`, F5-08), and non-nested tuple (`8b7d057`, F5-09) value parts are all now supported, closing the full interpolation-value-part matrix. Int/float share a `pebble_rt_str_from_parts` measure+`snprintf`-format refactor (scratch buffer widened to 320 bytes for float's worst case); str/char/enum/struct/tuple each reuse the corresponding value builder already used by bare `print`. F5-06's verification also surfaced print's own independent NUL-truncation defect for `str`, tracked and since resolved as F5-06b (`3d5c93a`) below | — |
...
| Tuple ordinal read of a `char`/`str` element | ~~checker accepts, Emit cleanly rejects each: `char` hits `buildCharOperand`'s `TuplePlace` gate, `str` hits the str-local initializer's `Load` gate~~ **Resolved (`bd84ee9`)** for the local-declaration shape. `str` in a VALUE position (a call argument) resolved separately (`9e6bcdc`, F5-15) — a `TuplePlace` branch added to `buildStrOperand` inline, verified across call-argument, local-init, comparison, and return-value positions. | **Closed** |
...
| Function value with C convention, variadic signature, or unsupported aggregate result | `validateFunctionTypeSignature` (`internal/backend/validate.go:306`) restricts the signature | **Partial** — float parameter/result widened (`cd6e604`); plain-struct parameter/result (F5-19/F5-20) and `str` result (F5-21) admitted. C convention, variadic, and any non-plain-struct aggregate parameter/result remain restricted |
...
| Tagged-union variant payload | any fixed-width integer at its own width, `bool`, `char`, `str`, float, a plain enum, a nested tagged union (`9a5342e`), or a *plain* struct (`ac60dc1`, F5-17 — self-contained scalar fields only, hoisted typedef between the enum and union blocks) | tuple, a struct with a nested aggregate field, pointer, array, slice, optional, and function value — each still needs an aggregate typedef the current union-leads-aggregate typedef order can't yet satisfy without a larger refactor |
...
| Fixed-array call argument | matching local, inline literal, scalar- or struct-element repeat (`cea6231`, F5-10), array-returning call, or whole dereference read | no current failure in these audited source shapes |
| Fixed-array return | matching local, inline literal, scalar- or struct-element repeat (`298dc80`, F5-11), array-returning call, or whole dereference read | no current failure in these audited source shapes |
...
| Optional argument/return | matching local, `some`, `none`, optional injection, optional-returning call, supported bare payload injection, or an optional field read (`8dff13c`, F5-16, both value/pointer receivers) | other optional-valued expressions need focused proof |
...
|---|---|---|
| Checked add, subtract, multiply | `i32`, `i64`, `u64`, `i8`, `i16`, `u8`, `u16`, `u32`; source `int` resolves to the entry signed width | **Resolved** (`afb8c77`, F6-03) — narrow widths now have real runtime helpers (SAFE-mode `__builtin_*_overflow` panic, RELEASE-mode wraparound), mirroring the i32/i64/u64 pattern. `uint` is unaffected by this row either way — it lowers to plain C arithmetic, not a checked helper call. |
| Checked divide and modulo | `i32`, `i64`, `u64`, `i8`, `i16`, `u8`, `u16`, `u32`; source `int` resolves to the entry signed width | **Resolved** (`ab67de5`, F6-04) — added `u64`/narrow-width helpers (`b == 0` always panics; signed narrow widths carry the same MIN/-1 overflow boundary as i32/i64, unsigned widths have none). |
| Checked shift left and right | `i32`, `i64`, `i8`, `i16`, `u8`, `u16`, `u32`, `uint`, `u64` — the full integer matrix | **Already complete, doc was stale** — `checkedShiftSuffix` (`internal/backend/operators.go`) and the matching runtime helpers already covered every width, confirmed 2026-08-14 by direct repro (`8u64 << 2u64` and `8uint << 2uint` both compile and run correctly, returning 32). No implementation work needed; only this row was out of date. |
| Checked integer negation | `int`, `i32`, `i64`, `i8`, `i16` — the full signed integer matrix (unsigned has no checked negation by design, see `pebble_rt.h`) | **Already complete, doc was stale** — `checkedNegSuffix` and the matching runtime helpers (`pebble_rt_checked_neg_i8`/`_i16`) already covered every signed width, confirmed 2026-08-14 by direct repro (a non-literal `i8` variable negation compiles and runs correctly). No implementation work needed; only this row was out of date. |
| Float-to-integer checked conversion | `int`, `i32`, `i64`, `i8`, `i16`, `u8`, `u16`, `u32`, `uint`, `u64` — the full integer destination matrix | **Already complete, doc was stale** — `floatToIntSuffix` and the matching runtime helpers already covered every integer destination width, confirmed 2026-08-14 by direct repro (`200.5f64 as u8` compiles and runs correctly, returning 200). No implementation work needed; only this row was out of date. |
