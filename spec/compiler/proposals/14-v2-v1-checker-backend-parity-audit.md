...
|---|---|---|---|---|
| F5-01 | Generic tagged-union method switch subject | A specialized method such as `Result[T, E].is_ok` keeps the template union TypeID on `self`. The backend union map is keyed by the concrete TypeID, so switch lowering misses the tagged-union path and emits the whole C union value instead of `.tag`. `std:result`'s `is_ok`, `unwrap_or`, and `map` fail end to end. | Fix only the specialized method receiver/switch-subject type substitution. Add a minimal generic method run test and one `std:result` run test. | **Backend semantic defect; highest priority** |
| F5-02 | Generic untagged-union field specialization | A generic untagged union instantiated with a scalar payload reaches Emit with the field type still recorded as a type parameter and is rejected as an unsupported C field shape. Non-generic scalar untagged unions work. | First trace the concrete member-type substitution boundary. Do not combine this with F5-01 unless one root cause is proven. | **Generic specialization defect** |
| F5-03 | `str` reassignment from another local | `var a str = "a"; var b str = "b"; a = b;` reaches `buildStoreCore`, whose `str` branch accepts only `StringLiteral` and `InterpolatedString`. V1 emits a normal string-view assignment. | Add only the `SymbolValue` source and its end-to-end test. | **Backend value-source gap** |
| F5-04 | `str` reassignment from a call | `s = get_str();` reaches the same branch and rejects `DirectCall`. | Delegate only `DirectCall`/`MethodCall` to `buildStrOperand`; test single evaluation. | **Backend value-source gap** |
| F5-05 | Interpolation of a `str` value part | General interpolated-string materialization rejects the part with “want bool, an integer type, or a float type”. V1 formats it. | Add only the `str` part ABI, runtime formatting, and end-to-end test. | **Backend/runtime formatting gap** |
| F5-06 | Interpolation of a `char` value part | Same rejection. V1 formats a character value. | Add only the `char` part. Include ASCII and multi-byte Unicode proof. | **Backend/runtime formatting gap** |
| F5-07 | Interpolation of an enum value part | Same rejection. V1 formats the variant name. | Add only the plain-enum part and invalid-tag defense. | **Backend/runtime formatting gap** |
| F5-08 | Interpolation of a struct value part | Same rejection. V1 recursively formats the value. | Add one non-nested struct part first. Keep nested recursion separate if required. | **Backend/runtime formatting gap** |
| F5-09 | Interpolation of a tuple value part | Same rejection. V1 recursively formats the value. | Add one scalar-element tuple part first. | **Backend/runtime formatting gap** |
| F5-10 | Aggregate `ArrayRepeat` call argument | Passing `[Point.{ ... }; N]` to `[N]Point` rejects the nominal element in `buildArrayArgument`. The scalar repeat path works, and V1 runs the aggregate case. | Route only a struct repeated value through `buildNestedAggregateValue`; keep return position separate. | **Backend aggregate-position gap** |
| F5-11 | Aggregate `ArrayRepeat` return | Returning `[Point.{ ... }; N]` calls the scalar expression builder and rejects `RecordConstruct`. V1 runs it. | Add only struct repeated values to `buildArrayReturnValue`; preserve evaluate-once behavior. | **Backend aggregate-position gap** |
| F5-12 | Whole tuple reassignment from a call | `pair = make_pair();` is explicitly rejected by `buildTupleStoreValue`; local and literal sources work. | Add only `DirectCall`, mirroring the resolved struct-store call path. | **Backend value-source gap** |
| F5-13 | Whole array reassignment from a call | `items = make_items();` is explicitly rejected by `buildArrayStoreValue`; local and literal sources work. | Add only `DirectCall` and copy from the returned wrapper without a second evaluation. | **Backend value-source gap** |
| F5-14 | Tuple-return forwarding | `return make_pair();` is rejected; tuple local and tuple literal returns work. Struct call forwarding already provides the nearest pattern. | Add only tuple-returning `DirectCall` in `buildAggregateReturnValue`. | **Backend return-source gap** |
| F5-15 | `str` tuple element as a call argument | A `str`-typed `Load(TuplePlace)` is rejected because `buildStrOperand` accepts only `CheckedIndexPlace` and `FieldPlace` loads. Local initialization from the same tuple element already works. | Add only `TuplePlace` to `buildStrOperand`. | **Backend value-source gap** |
| F5-16 | Optional field read as a call argument | Passing `holder.value` where the parameter type is optional reaches an optional-value builder that does not accept `Load(FieldPlace)`. | Add only the optional `FieldPlace` read, using the existing place-lvalue path. | **Backend value-source gap** |
| F5-17 | Struct payload in a tagged union | A variant with a struct payload gets a typedef-level or construction-level unsupported-payload rejection. Scalar, string, enum, and nested tagged-union payloads work. | Add only plain-struct payload C naming, dependency collection, construction, and narrowed read proof. | **Backend C-shape gap** |
| F5-18 | Fixed array of structs as a struct field | A field such as `items [N]Point` is rejected by the remaining array-through-aggregate ordering guard as “more than one level of nesting”. Arrays of arrays work; plain deep aggregate chains work. | Support only array-of-struct field ordering and construction. Keep tuple/optional elements separate. | **Backend typedef-order gap** |
| F5-19 | Aggregate parameter in a function value | A type such as `fn(Point) int` is rejected by `validateFunctionTypeSignature`. Ordinary helpers already pass structs by value. | Start with one plain-struct parameter. Update typedef collection and indirect-call argument lowering only for that shape. | **Backend function-value ABI gap** |
| F5-20 | Aggregate result in a function value | Aggregate results are rejected by the same first-class signature gate. | Start with one plain-struct result after F5-19. Do not combine parameter and result work. | **Backend function-value ABI gap** |
| F5-21 | `str` result in a function value | A type such as `fn() str` is rejected although ordinary helpers return `PebbleStr`. | Add only `str` result C naming and indirect-call result handling. | **Backend function-value ABI gap** |
...
| Interpolation | V1 formats string, bool, integer, float, char, enum, struct, and tuple parts and produces a string value | V2 materializes interpolated strings in general value positions and supports bool, integer, and float parts | **Partial** — `str`, `char`, enum, struct, and tuple parts remain open as F5-05 through F5-09 |
...
| General interpolated-string values | **Partial.** General materialization now works, with bool (`395eb2c`), integer (`7e4bcb4`), and float (`8dfcf51`) value parts. | F5-05 through F5-09, one part type per task |
