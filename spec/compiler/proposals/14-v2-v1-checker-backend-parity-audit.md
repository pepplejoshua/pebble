...
| **New, found while fixing the row above:** a tagged union used directly as a **switch subject** specifically (`switch pick()` where `pick()` returns a union) is handled by separate inline logic in `statements.go`, not `buildUnionValueExpr` — so that one specific position still rejects a direct call even after the row above. Narrower than originally scoped; `statements.go` untouched (out of that dispatch's declared scope). | high, confirmed empirically while fixing the row above | one reproduction: a tagged-union-returning call used directly as a switch subject, not bound to a local first |
