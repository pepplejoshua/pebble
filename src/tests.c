  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (parser.had_error) {
      ...
      for (size_t j = 0; j < program->data.block_stmt.stmt_count; j++) {
        ...
        switch (decl->kind) {
        case AST_DECL_FUNCTION:
          printf(" - %s (%zu params)", decl->data.func_decl.name,
...
static bool run_test_case(const char *test_name, TestCase tc) {
  ...
  bool success = collect_globals(program->data.block_stmt.stmts,
                                 program->data.block_stmt.stmt_count);
...
static void test_unique_globals(void) {
  ...
  TestCase tc = {.source = "fn add(a int, b int) int { return a + b; }\n"
...
static void test_duplicate_functions(void) {
  ...
  TestCase tc = {.source = "fn foo() int { return 1; }\n"
                           "fn foo() int { return 2; }",
                 .should_pass = false};
...
static void test_mixed_duplicates(void) {
  ...
  TestCase tc = {.source = "fn bar() void { }\n"
                           "var bar int;",
                 .should_pass = false};
...
static void test_type_resolution(void) {
  ...
  TestCase tc = {.source = "type MyInt = int;\n"
                           "type IntPtr = *int;\n"
                           "let pi = 3.14;\n"
                           "var count int;\n"
                           "var total = 100;\n"
...
static void test_tuples(void) {
  ...
  if (run_test_case("Tuples", tc)) {
    ...
    if (get_tuple && get_tuple->type &&
        get_tuple->type->kind == TYPE_FUNCTION) {
...
static void test_forward_references(void) {
  ...
  TestCase tc = {.source = "fn main() int { return helper(); }\n"
  ...
  if (run_test_case("Forward references", tc)) {
    ...
    if (result_type && result_type->type &&
        result_type->type->kind == TYPE_TUPLE) {
...
static void test_self_referential_direct(void) {
  ...
  TestCase tc = {.source = "type Node = Node;", .should_pass = false};
...
static void test_self_referential_via_pointer(void) {
  ...
  TestCase tc = {.source = "type Node = (int, *Node);\n"
                           "var head Node;",
                 .should_pass = true};
  ...
  if (run_test_case("Structural equality", tc)) {
    ...
    if (position && rgb) {
      ...
      printf("  %s Vec3 == Color: %s (both (int,int,int))\n",
             equal ? "✓" : "❌", equal ? "structurally equal" : "NOT equal");
    }
    ...
    if (c1 && c2) {
      ...
      printf("  %s ComplexA == ComplexB: %s\n", equal ? "✓" : "❌",
             equal ? "structurally equal (arrays of tuples + ptr)"
                   : "NOT equal");
    }
  }
}
...
static void test_structs(void) {
  ...
  if (run_test_case("Structs", tc)) {
    ...
    if (point_type && point_type->type &&
        point_type->type->kind == TYPE_STRUCT) {
...
static void test_function_types(void) {
  ...
  TestCase tc = {.source = "type BinaryOp = fn(int, int) int;\n"
  ...
  if (run_test_case("Function Types", tc)) {
    ...
    if (binary_op && binary_op->type &&
        binary_op->type->kind == TYPE_FUNCTION) {
...
static void test_slicing(void) {
  ...
  TestCase tc = {.source = "fn test_slicing() void {\n"
  ...
  if (run_test_case("Slicing", tc)) {
    ...
    if (take_slice && take_slice->type &&
        take_slice->type->kind == TYPE_FUNCTION) {
...
static void test_pointer_operators(void) {
  ...
  TestCase tc_valid = {.source = "fn test_pointers() void {\n"
                                 "  var x int = 42;\n"
                                 "  var ptr *int = &x;\n"
                                 "  var val int = *ptr;\n"
                                 "  var ptr_ptr **int = &ptr;\n"
  ...
  if (run_test_case("Pointer Operators - Valid", tc_valid)) {
    ...
    if (get_pointer && get_pointer->type &&
        get_pointer->type->kind == TYPE_FUNCTION) {
  ...
  TestCase tc_invalid = {.source = "fn test_invalid() void {\n"
                                   "  var ptr *int = &5;\n"
                                   "}",
                         .should_pass = false};
...
static void test_array_literals(void) {
  ...
  TestCase tc = {.source = "fn test_arrays() void {\n"
                           "  var nums = [1, 2, 3, 4, 5];\n"
  ...
  if (run_test_case("Array Literals", tc)) {
    ...
    if (make_array && make_array->type &&
        make_array->type->kind == TYPE_FUNCTION) {
