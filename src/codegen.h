...
typedef struct {
    ...
    char *preamble;
    char *forward_types; size_t forward_types_len, forward_types_cap;
    char *type_defs; size_t type_defs_len, type_defs_cap;
    char *forward_vars_funcs; size_t forward_vars_funcs_len, forward_vars_funcs_cap;
    char *defs; size_t defs_len, defs_cap;
...
void emit_to_section(Codegen *cg, const char *section, const char *str);  // New: Append to section

// Helpers for types and deduplication
void emit_type_if_needed(Codegen *cg, Type *type);  // Check/gen forward/def if not declared/defined
